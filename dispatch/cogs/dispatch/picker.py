...
class AirportFilter:
    ...
    def __init__(self, types: Set[str] = set(), continent: Optional[str] = None):
        self.types = types
        self.continent = continent

    def airport_filter(self, x: Airport) -> bool:
        if self.continent and x.continent != self.continent:
            return False
...
class AirportPairPicker:
    ...
    def __init__(self, airports: Airports, aircrafts: aircraft_lib.Aircrafts):
        ...
        self.airport_filter = AirportFilter(
...
class AirportPairPickerVatsim(AirportPairPicker):
    ...
    def pick_pair(
        ...
        dbg_hash = dict()
        self.kept = []
        self.score_airports_vatsim(continent)
        min_distance_nm, max_distance_nm = self.distances_from_type(hours)
        print(
    ...
    def score_airports_vatsim(self, continent: str):
        score_buckets: Dict[int, List[Airport]] = {}
        for airport in self.airports.airports:
            if continent is None or airport.continent == continent:
