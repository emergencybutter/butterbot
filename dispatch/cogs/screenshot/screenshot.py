...
VOYAGER_BLUE = discord.Color.from_str("#4daacf")
VOYAGER_ORANGE = discord.Color.from_str("#e87d1e")
GREEN = discord.Color.from_str("#00ff00")
...
def is_voyager_emoji(emoji):
    return getattr(emoji, 'id', 0) == _VOY_EMOJI or str(emoji) == '▫️'
...
class Screenshot(commands.Cog):
    ...
    async def process_reactions_last_month(self, channel, nowutc):
        ...
        async for message in channel.history(limit=None, after=start_time, before=end_time):
            ...
            for x in message.reactions:
                ...
                if not is_voyager_emoji(x.emoji):
                    continue
                if x.count > top_reaction_count:
                    top_message = message
                    top_reaction_count = x.count
        ...
        embed = discord.Embed(title=f'Winning screenshot of {month_name}', description=f'This past month we saw {num_messages} messages, {num_images} images and {num_reactions} reactions in this channel. This image got the most <:Voyager2:1064727097421090817> reactions! Congrats to <@{top_message.author.id}>', color=VOYAGER_BLUE, url=top_message.jump_url)
        embed.set_image(url=url)
        embed.set_footer(text='ScreenshotBot (c) Voyager Aviation', icon_url='https://flyvoyager.net/Voyager_Logo_Transparent.png')
        embed.set_author(name='ScreenshotBot', url='https://flyvoyager.net/pictures.html', icon_url='https://flyvoyager.net/Voyager_Logo_Transparent.png')
    ...
    async def on_reaction_add(self, reaction, user):
        if reaction.message.channel.id not in [_SCREENSHOT_ID, _BOT_TEST_ID, _EMERGENCY_SERVER_SCREENSHOT_ID]:
            return
        ...
        for attachment in reaction.message.attachments:
            ...
            await reaction.message.attachments[0].save(os.path.join(_IMAGE_DIR, x))
        genimagejson()
